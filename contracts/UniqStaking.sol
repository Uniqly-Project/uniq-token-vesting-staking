...
contract UniqStaking {
    ...
    struct PoolInfo {
        ...
        address requiredErc20Token;
        uint requiredErc20TokenQuantity;
        address requiredErc721Token;
        uint requiredErc721TokenQuantity;
        string image;
        string name;
    }

    struct RequiredValuesOfStake {
        address requiredErc20Token;
        uint requiredErc20TokenQuantity;
        address requiredErc721Token;
        uint requiredErc721TokenQuantity;
    ...
    */
    ...
    @dev slots: max number of stake users
    @dev stakeValue: min token required
    @dev closeTime: lifetime of a pool in seconds + now
    @dev lockPeriod: time to stake (in seconds)
    @dev image: URL of image gift
    @dev name: staking name
    @dev usedSlots: initialize zero
   */
    function addStakePool(
        ...
        RequiredValuesOfStake calldata requiedValues
    ) external onlyOwner {
        _poolInfo.push(
            PoolInfo({
                slots: _slots,
                ...
                image: _image,
                name: _name,
                requiredErc20Token: requiedValues.requiredErc20Token,
                requiredErc20TokenQuantity: requiedValues.requiredErc20TokenQuantity,
                requiredErc721Token: requiedValues.requiredErc721Token,
                requiredErc721TokenQuantity: requiedValues.requiredErc721TokenQuantity
            })
        );
    }
   ...
   */
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        require(
            IERC20(pool.requiredErc20Token).balanceOf(msg.sender) >= pool.requiredErc721TokenQuantity,
            "UniqStaking.deposit: not enough required Erc20 token"
        );
        require(
            IERC721(pool.requiredErc20Token).balanceOf(msg.sender) >= pool.requiredErc721TokenQuantity,
            "UniqStaking.deposit: not enough required Erc20 token"
        );
